...
1.  **In-Process Handoff**: Keep agent communication in-process by default (an ADK 2.0 `Workflow` graph inside a single process — see `docs/adr/0004`; `AgentTool` is discouraged and `SequentialAgent`/`LoopAgent` are deprecated in google-adk 2.x). Do not add distributed infrastructure (e.g. gateways, pub/sub, HTTP routing) to the core pipeline.
