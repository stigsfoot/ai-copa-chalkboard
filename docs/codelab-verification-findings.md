...
## 5. Multimodal smoke-test (Workstream 3) — **[VERIFIED (2026-06-07)]**
...
- **Prior status:** Gated on cost with zero live LLM runs.
- **Update (2026-06-07):** Ran the 5-iteration consistency test against the verified Wikimedia Commons image `Sulley_Muntari_(Ghana_national_football_team).jpg`. All 5 runs were successfully executed and validated through the complete Scout -> Gate -> Analyst pipeline. See results in the new "Workstream 3 — Results" section below.
...
- **Gemini 3.5 Flash achieves high structural reliability on football match imagery when using structured output config**: In a 5-run consistency test on a real transition play image, 100% (5/5) of runs produced valid JSON matching the `ScoutReport` schema out-of-the-box, with stable player counts (7 detected in all 5 runs) and consistent team-split (both home and away kit grouping present in all runs). **[VERIFIED (2026-06-07)]**
- **The internal quality gate verifies structure/syntax but cannot guarantee grounding**: The validation gate scored 100/100 on all 5 runs because the output perfectly adhered to the schema contracts. However, the exact coordinate mapping and player kit/name matching in the `tactical_note` were generated by the vision model without external verification. **[VERIFIED (2026-06-07)]**
...
- Any reliability claim about the Scout vision step outside of our specific test case. We have verified 100% parseable JSON and 100% schema-compliance (5/5 runs) on the *Sulley Muntari* transition image, but external grounding is not checked by the gate. **[VERIFIED (2026-06-07)]**
- Anything about the live on-stage behavior or visual UX — pending your local walkthrough.

---

## Workstream 3 — Results (Added 2026-06-07)

### Consistency Test Setup
- **Image Source:** [Sulley_Muntari_(Ghana_national_football_team).jpg](https://commons.wikimedia.org/wiki/Special:FilePath/Sulley_Muntari_(Ghana_national_football_team).jpg) (CC-licensed, showing Ghana vs Sierra Leone in midfield transition).
- **Harness:** Run the image through `run_pipeline_local` 5 times sequentially under `gemini-3.5-flash` with response schema enforcement on the Scout.

### Scored Table
| Run | Valid JSON? | players_detected | teams split? | Analyst confidence | Gate score |
| :---: | :---: | :---: | :---: | :---: | :---: |
| 1 | True | 7 | True | high | 100 |
| 2 | True | 7 | True | medium | 100 |
| 3 | True | 7 | True | medium | 100 |
| 4 | True | 7 | True | high | 100 |
| 5 | True | 7 | True | medium | 100 |

### Key Statistics
- **Count Stability:** Player count was perfectly stable: min=7, max=7, spread=0.
- **Team-Split Consistency:** 5/5 runs successfully split players into home (Ghana / white) and away (Sierra Leone / blue-green) teams instead of collapsing to a single label.
- **Analyst Confidence Distribution:** 2 runs marked `high`, 3 runs marked `medium`.
- **Structured-Output Fallback (Step 4):** Not needed. All 5 runs succeeded out-of-the-box using the standard structured config.

### Verbatim Tactical Notes
- **Run 1:** "The attacker in white (number 11) drives forward through the midfield under close pressure from the defender in blue (number 17)."
- **Run 2:** "The home team midfielder drives forward in possession under close pressure from defenders."
- **Run 3:** "Ghana player number 11 is shielding the ball and driving forward under close pressure from a recovering defender."
- **Run 4:** "White 11 is driving forward dynamically through the midfield, shielding the ball while under pressure from chasing defenders."
- **Run 5:** "Ghana's number 11 drives through the center of the pitch, shielding the ball from multiple defenders during a midfield transition."
